...
class CLIPVisionTower(nn.Module):
    ...
    def token_prune_merge_advanced(self, images, if_adaptive=True, reduction_ratio = 1/8):
        ...
        col_ln_attn = torch.norm(attn, p=4, dim=1)
        col_ln_attn = col_ln_attn / (col_ln_attn.sum(dim=1, keepdim=True) + 1e-6)
        col_ln_attn = col_ln_attn[:, 1:]

        if if_adaptive:
            reduction_ratio = outlier_dectection(col_ln_attn)#*3.5
        _, idx = torch.topk(col_ln_attn, int(N*reduction_ratio), dim=1, largest=True)  # [B, left_tokens] , sorted=True
        index = idx.unsqueeze(-1).expand(-1, -1, C)  # [B, left_tokens, C]

        Key_wo_cls = desired_layer_k[:, 1:]  # [B, N-1, C]

        x_others = torch.gather(image_features, dim=1, index=index)  # [B, left_tokens, C]
        x_others_attn = torch.gather(col_ln_attn, dim=1, index=idx)  
        ...
        non_topk_attn = torch.gather(col_ln_attn, dim=1, index=compl)  # [B, N-1-left_tokens]
    ...
    def forward(self, images):
        if type(images) is list:
            ...
            image_features = self.token_prune_merge_advanced(images, if_adaptive=False, reduction_ratio=3/9)
            
            # image_features = self.token_prune_merge_advanced_plus(images, if_adaptive=False, reduction_ratio=1/8) 
