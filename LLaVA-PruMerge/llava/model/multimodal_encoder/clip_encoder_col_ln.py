...
class CLIPVisionTower(nn.Module):
    ...
    def token_prune_merge_advanced(self, images, if_adaptive=True, reduction_ratio = 1/8):
        ...
        if if_adaptive:
            reduction_ratio = outlier_dectection(col_ln_attn)
        _, idx = torch.topk(col_ln_attn, int(N*reduction_ratio), dim=1, largest=True)
        ...
        non_topk_attn = torch.gather(col_ln_attn, dim=1, index=compl)
