...
class Attention(nn.Module):
    ...
    def forward(self, x, keep_rate=None, tokens=None):
        ...
        if self.keep_rate < 1 and keep_rate < 1 or tokens is not None:
            ...
            B, N_minus_1 = attn.shape[0], attn.shape[3] - 1

            random_scores = torch.rand(B, N_minus_1, device=attn.device)
            random_scores = random_scores / (random_scores.sum(dim=1, keepdim=True) + 1e-6)
            _, idx = torch.topk(random_scores, left_tokens, dim=1, largest=True, sorted=True)

            index = idx.unsqueeze(-1).expand(-1, -1, C)

            return x, index, idx, random_scores, left_tokens
...
@register_model
def vit_small_patch16_augreg_evit(pretrained=False, base_keep_rate=0.7, drop_loc=(3,6,9), **kwargs):
    ...
    for loc in drop_loc:
        model_kwargs['keep_rate'][loc] = base_keep_rate
...
def vit_base_patch16_augreg_evit(pretrained=False, base_keep_rate=0.7, drop_loc=(3, 6, 9), **kwargs):
    ...
    model = _create_evit('vit_base_patch16_augreg_evit', pretrained=pretrained, **model_kwargs)
    return model